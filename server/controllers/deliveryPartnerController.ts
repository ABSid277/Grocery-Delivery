...
export const getDeliveryDetail = async (req: Request, res: Response) => {
    const order = await prisma.order.findFirst({
        where: {id: req.params.id as string, deliveryPartnerId: req.
        partner!.id},
        include: {user: {select: {name: true, email: true, phone: true}}}
    })
...
export const cancelDelivery = async (req: Request, res: Response) => {
    ...
    history.push({status: 'Cancelled', note: reason || "", timestamp: new 
    Date()})
...
export const updateDeliveryStatus = async (req: Request, res: Response) => {
    ...
    history.push({status, note: `Status updated to ${status}`, timestamp: 
    new Date()})
